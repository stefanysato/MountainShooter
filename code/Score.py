    ...
    def save(self, game_mode: str, player_score: list[int]):
        ...
        while True:
            ...
            if game_mode == MENU_OPTION[0]:  # 1P
                ...
                text = "Enter P1 name (4 characters)"
            if game_mode == MENU_OPTION[1]:  # 1P
                score = (player_score[0] + player_score[1]) / 2
                text = "Enter team name (4 characters)"
            if game_mode == MENU_OPTION[2]:  # 1P
                if player_score[0] >= player_score[1]:
                    score = player_score[0]
                    text = "Enter P1 name (4 characters)"
                else:
                    score = player_score[1]
                    text = "Enter P2 name (4 characters)"
            self.score_text(28, text, C_WHITE, SCORE_POS['EnterName'])
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RETURN and len(name) == 4:  # enter
